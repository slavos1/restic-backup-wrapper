...
`skip-if-unchanged` {opt}::
Boolean.
If set to `true` (default) then https://github.com/restic/restic/issues/662[`--skip-if-unchanged`] will be added to all `restic backup` calls.
Must be set to `false` for `restic` prior to `0.17.0`.
