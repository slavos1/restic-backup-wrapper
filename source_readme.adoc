...
*{cli}* is a command-line tool that generates `{rt} backup` commands from a TOML config file.
...
{cli} generates https://restic.readthedocs.io/en/latest/040_backup.html[`{rt} backup`] commands (logs not shown):
...
When writing documentation, I run `make doc-devel` and preview the generated HTML.

Once ready to commit, I run `make doc` to generate link:README.md[README.md] file from link:source_readme.adoc[source_readme.adoc] (and included files).
